...
/// Sent to any account holder whenever their password is set or changed —
/// guest, family, or admin.
///
...
/// in the real account holder's inbox, which is the one place they would
/// notice.
pub fn password_changed(app_url: &str) -> Email {
    let subject = "Your Dulac My Camp password was changed".to_string();
    let body = format!(
        r#"<p style="margin:0 0 18px;">The password on your Dulac My Camp account was just set or changed.</p>
